...
namespace SoundCatcher //тут можно менять (сужжать, расширять осциллограмы, менять цвет) формат/вид осциллографа
                        // этот класс используется для осциллографа
{
    class AudioFrame
    {
        ...
        public void RenderTimeDomainLeft(ref PictureBox pictureBox)
        {
            ...
            // Draw left channel
            double scale = 0.5 * height / 32768 * 0.90;  // a 16 bit sample has values from -32768 to 32767 //высоту менять вроде
            int xPrev = 0, yPrev = 0;
            for (int x = 0; x < width; x += 10) //тут меняется ширина осциллограм { x+=5 }
            {
                ...
                {
                    pen.Color = Color.Blue; //меняется цвет осцилограм
                    //pen.Color = Color.FromArgb(49, 254, 27);
                    pen.Width = 2; //толщина
        ...
        public void RenderTimeDomainRight(ref PictureBox pictureBox)
        {
            ...
            for (int x = 0; x < width; x+=10) //тут меняется ширина осциллограм { x+=5 }
            {
                ...
                {
                    pen.Color = Color.Red; //меняется цвет осцилограм
                    pen.Width = 2; //толщина
                    offScreenDC.DrawLine(pen, xPrev, yPrev, x, y);
                    xPrev = x;
                    yPrev = y;
